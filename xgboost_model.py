...
def plot_results(y_test, y_pred, y_prob, model, feature_cols):
    ...
    print("混淆矩阵和ROC曲线已保存: results_xgboost_evaluation.png")
    plt.close()
    ...
    print("特征重要性图已保存: results_xgboost_feature_importance.png")
    plt.close()
...
def main():
    ...
    # 3. 训练XGBoost模型
    model, train_time = train_xgboost(X_train, y_train, X_test, y_test)
