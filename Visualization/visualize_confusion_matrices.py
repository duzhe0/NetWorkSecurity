#!/usr/bin/env python3
"""
XGBoost / DNN 混淆矩阵可视化（可切换内部测试集 / 外部测试集）

用法:
    python visualize_confusion_matrices.py                     # 默认：KDDTrain 内部测试集
...
def load_confusion_matrix(model_name, source='internal'):
    """
    从 CSV 加载并解析混淆矩阵。
    ...
    if source == 'internal':
        ...
        filename = f'results_{model_name}_external_test.csv'
...
def main():
    parser = argparse.ArgumentParser(description='XGBoost / DNN 混淆矩阵可视化')
