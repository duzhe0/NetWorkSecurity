...
def evaluate_external_test_dnn(model, num_classes, class_names, device, conf_threshold=0.5):
    """在外部 train_test 集上评估 DNN 模型（零信息泄露）
    
    conf_threshold: 置信度阈值，max_prob < 阈值则标记为"未知"（不强制分类）
    """
    print("\n" + "=" * 60)
...
def main():
    ...
    # 打印汇总表
    print("\n" + "=" * 100)
    print("DNN 多方案双轨评估汇总（含置信度拒绝）")
    print("=" * 100)
    print(f"{'方案':<12} {'内部Acc':>8} {'内部F1':>8} {'外部全量Acc':>10} {'拒绝数':>6} {'高置信Acc':>10} {'高置信F1':>8} {'耗时(s)':>8}")
    print("-" * 100)
    for r in all_results:
        print(f"{r['scheme']:<12} {r['internal_acc']:>8.4f} {r['internal_f1']:>8.4f} "
              f"{r['external_acc']:>10.4f} {r['n_rejected']:>6d} "
              f"{r['external_acc_conf']:>10.4f} {r['external_f1_conf']:>8.4f} "
              f"{r['train_time']:>8.1f}")
