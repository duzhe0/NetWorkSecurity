...
def train_transformer(X_train, X_val, y_train, y_val, num_classes, feature_cols, epochs=50, batch_size=64,
                      weight_scheme='none'):
    ...
    base_model = TransformerClassifier(embedded_dim, num_classes, d_model, nhead,
                                       num_layers, dim_feedforward, dropout_rate)
...
def evaluate_external_test_transformer(model, num_classes, class_names, device, conf_threshold=0.5):
...
def main():
    ...
    print("\n" + "=" * 110)
    print("Transformer 多方案双轨评估汇总（含置信度拒绝）")
    print("=" * 110)
